#Eigenvalue problem--TISE
from numpy import array, arange, dot, sqrt
from pylab import plot, show, xlim, xlabel, ylabel, legend

'''
-(hbar/2m)D^2(\psi)+V(x)(\psi)=E(\psi)
'''

m=9.1094e-31 #mass of electron
hbar=1.0546e-34
e=1.6022e-19
L=5.2918e-11 #Bohr radi.
N=1000
h=L/N

#Potential
def V(x):
    return 0.0

def f(r,x,E):
    psi=r[0]
    phi=r[1]
    fpsi=phi
    fphi=(2*m/hbar**2)*(V(x)-E)*psi
    return array([fpsi,fphi],float)
#Calculate the wavefunction for a particular energy

def RK4(r,x,E):
    k1=h*f(r,x,E)
    k2=h*f(r+0.5*k1,x+0.5*h,E)
    k3=h*f(r+0.5*k2,x+0.5*h,E)
    k4=h*f(r+k3,x+h,E)
    r+=(k1+2*k2+2*k3+k4)/6
    return r

def solve(E):
    psi=0.0
    phi=1.0
    r=array([psi,phi],float)

    for x in arange(0,L,h): #0부 터 L까  지 h간격으 로 자르 기   
        r=RK4(r,x,E)
    return r[0]

#Main program to find the energy using the secant method
E1=0
E2=e
psi2=solve(E1)

tolerance=e/1000
while abs(E2-E1)>tolerance:
    psi1, psi2 = psi2, solve(E2)
    E1,E2=E2,E2-psi2*(E2-E1)/(psi2-psi1)

print('E=',E2/e, 'eV')

#calculate the psi

ppsi=[]
pphi=[]
px=[]

ppsi.append(0.0)
pphi.append(1.0)
px.append(0.0)

r=array([ppsi[0],pphi[0]],float)
for x in arange(0,L,h):
    r=RK4(r,x,E2)
    ppsi.append(r[0])
    pphi.append(r[1])
    px.append(x+h)

#Normalize psi
integ=0.0
for i in range(len(px)):
    integ+=h*ppsi[i]**2  # integral[\psi(x),(x,0,L)]
norm_ppsi=ppsi/sqrt(integ)

plot(px,norm_ppsi)
xlim(-L,2*L)
show()

